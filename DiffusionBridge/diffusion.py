...
class model(torch.nn.Module):
        ...
        for m in range(M, 0, -1):
            ...
            if (m > 1):
                if modify:
                    ...
                    scaling = stepsize
                brownian = torch.sqrt(scaling) * torch.randn(Z.shape).to(device) # size (N x d)
                Z = euler + sigma * brownian
                trajectories[:, m-1, :] = Z
                scaled_brownian[:, m-1, :] = - (invSigma / scaling) * sigma * brownian
            else:
                # terminal constraint
                if modify:
                    ...
                    scaling = stepsize
                trajectories[:, 0, :] = X0
                scaled_brownian[:, 0, :] = - (invSigma / scaling) * (X0 - euler)
    ...
    def my_learn_full_score_transition(self, simulate_initial_state, simulate_terminal_state, epsilon, minibatch, num_initial_per_batch, num_iterations, learning_rate, ema_momentum, scheduler=None, device='cpu'):
        """
        Learn full approximation of score transition using score matching.
        ...
        # create score network
        score_net = FullScoreNetwork(dimension = self.d).to(device)
        score_net.train()
        ema_parameters = ema_register(score_net)
        ...
        with Progress(
            ...
            task1 = progress.add_task("[red]Training full_score_transition (lr: X) (loss=X)", total=num_iterations)
    ...
    def my_learn_score_marginal(self, score_transition_net, simulate_initial_state, simulate_terminal_state, epsilon, minibatch, num_initial_per_batch, num_iterations, learning_rate, ema_momentum, num_workers=0, device='cpu', scheduler=None, checkpoint=None):
        ...
        with Progress(
            ...
            task1 = progress.add_task("[red]Training score marginal (lr: X) (loss=X)", total=num_iterations)
            while not progress.finished:
                for trajectories, scaled_brownian in dl:
                    ...
                    for j in range(num_batches):
                        ...
                        score = score_net(timesteps_flatten, traj_flatten, initial_states_flatten, terminal_states_flatten) # size (N*M, d)
                        # score = score_net(timesteps_flatten, traj_flatten, initial_states_flatten) # size (N*M, d)
                    ...
                    cur_lr = optimizer.param_groups[-1]['lr']
                    progress.update(task1, advance=1, description="[red]Training score marginal (lr: %2.5f) (loss=%2.5f)" % (cur_lr, current_loss))
                    # if iter_nums % 10 == 0:
    ...
    def learn_score_marginal(self, score_transition_net, initial_state, terminal_state, epsilon, minibatch, num_iterations, learning_rate, ema_momentum, full_score=False):
        ...
        for i in range(num_repeats):            
            ...
            with torch.no_grad():
                simulation_output = self.simulate_bridge_backwards(score_transition_net, initial_state, terminal_state, epsilon, num_samples, modify = True, full_score = full_score)            
            trajectories = simulation_output['trajectories']
            scaled_brownian = simulation_output['scaled_brownian']
    ...
    def load_checkpoint_score(self, filename):
        """
